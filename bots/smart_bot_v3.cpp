...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        if (cmd == "GAME_START") {
            ...
            if (gs.board.empty()) {
                // shouldn't happen (vote is only post-flop) but just in case
                std::cout << "VOTE YES 0" << std::endl;
            } else {
                double equity = estimate_equity(gs.hole[0], gs.hole[1], gs.board,
                                                std::max(1, num_opp), 150, rng, gs.mc_dead());

                //TODO: 0.55 and 0.30 as hyperparameters
                if (equity >= 0.55) {
                    // strong hand — vote YES to keep, wager proportional to strength
                    int wager = (int)(my_chips * std::min(0.15, (equity - 0.5) * 0.5));
                    wager = std::max(0, std::min(wager, my_chips));
                    std::cout << "VOTE YES " << wager << std::endl;
                } else if (equity < 0.30) {
                    // weak hand — vote NO to redraw, wager proportional to weakness
                    int wager = (int)(my_chips * std::min(0.10, (0.35 - equity) * 0.3));
                    wager = std::max(0, std::min(wager, my_chips));
                    std::cout << "VOTE NO " << wager << std::endl;
                } else {
                    // medium hand — vote YES with minimal wager (prefer stability)
                    std::cout << "VOTE YES 0" << std::endl;
                }
            }
        }
